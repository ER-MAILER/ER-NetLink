...
class SessionManager(QObject):
    ...
    def __init__(self):
        ...
        self.user_status_listener = None
    ...
    def initialize_session(self, user):
        try:
            ...
            self.session_ref = self.db.reference(f"users/{uid}/sessions/{self.device_id}")
            self.session_ref.set(session_data)
            
            self.update_timer.start(30000)  # Update every 30 seconds
            
            self._monitor_user_status(uid, user['idToken'])
            self._monitor_session_status(uid, user['idToken'])
    ...
    def cleanup_on_exit(self):
        if not self.cleanup_done:
            self.logout(normal_logout=False)
            self.cleanup_done = True

    def update_last_active(self):
        if self.current_user and self.session_ref:
            try:
                self.session_ref.update({
                    'last_active': int(time.time() * 1000)
                })
            except Exception as e:
                print(f"Failed to update last active time: {str(e)}")

    def _monitor_user_status(self, uid, id_token):
        def user_status_handler(message):
            try:
                if message.event_type == 'put':
                    if message.path == '/':
                        if message.data is None:
                            self.logout_signal.emit("Your account has been deleted by admin")
                    elif message.path == '/status' and message.data == 'inactive':
                        self.logout_signal.emit("Your account has been disabled by admin")
            except Exception as e:
                print(f"User status handler error: {str(e)}")
        
        try:
            self.user_status_listener = self.db.reference(f"users/{uid}").listen(user_status_handler)
        except Exception as e:
            print(f"User status monitor error: {str(e)}")

    def _monitor_session_status(self, uid, id_token):
        def session_status_handler(message):
            try:
                if message.event_type == 'put':
                    if message.data is None or message.data.get('status') == 'terminated':
                        self.logout_signal.emit("Your session has been terminated")
            except Exception as e:
                print(f"Session status handler error: {str(e)}")
        
        try:
            self.session_listener = self.db.reference(f"users/{uid}/sessions/{self.device_id}").listen(session_status_handler)
        except Exception as e:
            print(f"Session monitor error: {str(e)}")
    ...
    def logout(self, normal_logout=True):
        try:
            ...
            if self.user_status_listener:
                self.user_status_listener.close()
                
            if self.session_listener:
                self.session_listener.close()
            
            if not self.cleanup_done and self.current_user and self.session_ref:
                self.session_ref.update({
                    'status': 'inactive',
                    'last_active': int(time.time() * 1000)
                })
...
class LoginDialog(QDialog):
    ...
    def check_credentials(self):
        ...
        try:
            ...
            sessions_ref = self.session_manager.db.reference(f"users/{uid}/sessions")
            ...
            if active_sessions:
                reply = QMessageBox.question(
                    ...
                    "You're already logged in on another device.\nDo you want to terminate other sessions?",
                    QMessageBox.Yes | QMessageBox.No
                )
                
                if reply == QMessageBox.Yes:
                    for session_id in active_sessions:
                        sessions_ref.child(session_id).update({'status': 'terminated'})
                else:
                    return
...
if __name__ == "__main__":
    ...
    # Set application icon for taskbar and all windows
    app.setWindowIcon(QIcon('ec\\icon.ico'))

    # Show login dialog
    login_dialog = LoginDialog()
    if login_dialog.exec_() == QDialog.Accepted:
        window = EmailSenderApp(login_dialog.session_manager)
        window.show()
        sys.exit(app.exec_())
    else:
        sys.exit()
